    ...
    if (empty($tenchude)) {
        ...
        try {
            ...
            if ($checkCmd->fetch()['total'] > 0) {
                ...
                $cmd->bindValue(2, $ngaytao, PDO::PARAM_STR);
                $cmd->execute();
            ...
            $message = 'Lỗi khi thêm chủ đề: ' . $e->getMessage();
            $message_type = 'danger';
        }
    }
}
                        ...
                        <form method="post" class="mb-4">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="tenchude" name="tenchude"
                                    value="<?php echo isset($_POST['tenchude']) ? htmlspecialchars($_POST['tenchude']) : ''; ?>"
                                    required>
                            </div>
                            <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Lưu danh
                                mục</button>
                        </form>
                    </div>
                </div>
